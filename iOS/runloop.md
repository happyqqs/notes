...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    __CFLock(&loopsLock);
    if (!__CFRunLoops) {
        // 第一次进入时，初始化全局dict，并先为主线程创建一个 RunLoop。
        __CFUnlock(&loopsLock);
        CFMutableDictionaryRef dict = CFDictionaryCreateMutable(kCFAllocatorSystemDefault, 0, NULL, &kCFTypeDictionaryValueCallBacks);
        CFRunLoopRef mainLoop = __CFRunLoopCreate(pthread_main_thread_np());
        CFDictionarySetValue(dict, pthreadPointer(pthread_main_thread_np()), mainLoop);
        // 先判断全局字典是否为NULL，如果是则替换为局部字典，，这虽然是两步操作，但是系统保证他们是一个原子操作，所以可以在不加锁的情况下执行
    ...
    __CFUnlock(&loopsLock);
    if (!loop) {
        /// 取不到时，创建一个  猜测是创建循环的时间比较长，，不希望在临界区停留时间这么久吧
...
第一次从获取全局字典是要加锁的 ，接下来获取运行循环也是要加锁的 ，这两个加的一个锁 ，如果说第二步的创建一个运行循环占用时间太长，会阻塞其他线程在第一步的加锁操作 ，所以后面需要解锁创建运行循环然后再次加锁判断是否已经存在运行循环
...
![avatar](https://blog.ibireme.com/wp-content/uploads/2015/05/RunLoop_3.png)
